...
import moon from '../assets/heart.jpg'

export const MovieDetail = () => {
  const params = useParams();
  const [movie,setMovie] = useState({});
  const image = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : moon;



  useEffect(() => {
    async function fetchMovie(){
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=9ea8dd685af411aab84e28444cc5e02c`);
    ...
    <main>
      <section className='flex justify-around flex-wrap py-5'>
        ...
        <p className='my-4'>
          ...
          <a href={`https://www.imdb.com/title/${movie.imdb_id}`} target='blank' rel="noreferrer">{movie.imdb_id}</a>
        </p>
